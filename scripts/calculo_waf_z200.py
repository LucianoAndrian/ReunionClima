...
def descarga_nc( mesi, diai, mesf, diaf, aniof, variable_entrada, variable_salida, tipo):
    ...
    if tipo==3 and mesf==2 and diaf == 29:
        diaf=28 # no hay climatologia con febrero bisiesto
    ...
    if not os.path.exists(ruta):
        os.mkdir(ruta)
    print('http://www.psl.noaa.gov'+"".join(link), ruta+variable_salida+'.nc')
    urllib.request.urlretrieve('http://www.psl.noaa.gov'+"".join(link), ruta+variable_salida+'.nc')

# In[]      Downloads netcdf from esrl    
def descarga_nc_z(inid, inim, find, finm, finy, var_desc,var, level):
...
def main():
    ...
    var='hgt'
    level='200mb'
    descarga_nc_z(inid, inim, find, finm, finy, var_desc,var, level)
    ...
    [anomvar,lonz,latz] = manipular_nc(ruta+var+'.nc',var)
...
#========================================================================================    
    ...
    lonsz, latsz = np.meshgrid(lonz,latz)
    clevs = np.arange(-2.5e7,2.75e7,0.25e7)
    clevs = np.arange(-150,150,15)
    
    import matplotlib
    if int(matplotlib.__version__[2]) > 7:
    ...
    im=ax.contourf(lonsz, latsz, np.squeeze(anomvar), clevs, transform=crs_latlon, cmap=cmap, extend='both')
    #contour levels
    ax.contour(lonsz, latsz, np.squeeze(anomvar), clevs, colors='k', linewidths=0.5, transform=crs_latlon)
    ...
    ax.set_title('Anomalías hgt 200hPa y Flujos de Plumb '+str(inid)+'/'+str(inim)+'/'+str(iniy)+'-'+str(find)+'/'+str(finm)+'/'+str(finy))
    
    #save figure
    fig1.savefig('Z_plumb_'+'{:02d}'.format(inid)+'{:02d}'.format(inim)+str(iniy)+'-'+'{:02d}'.format(find)+'{:02d}'.format(finm)+str(finy)+'.png',dpi=300,bbox_inches='tight',orientation='landscape')
